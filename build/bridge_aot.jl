    ...
    if exc isa CompositeException
        ...
        if !isempty(excs)
            ...
            if tfe isa TaskFailedException
                ...
                ccall(:_write, Cint, (Cint, Ptr{UInt8}, Cuint), Int32(2), " [inner: ", Cuint(9))
                ccall(:_write, Cint, (Cint, Ptr{UInt8}, Cuint), Int32(2), icstr, Cuint(ilen))
...
const _INIT_COVER_FRACTION = 0.3f0
...
# mean_colony_cover_m2() is called here at DLL call-time, not at module load time,
# because it calls erfcx (via SpecialFunctions) which requires a native library that
# is unavailable during JuliaC AOT compilation.
function _build_ensemble_params(area_m2::Float32, n_members::Int)::Matrix{Float64}
    mean_cov = Float64(Kora.mean_colony_cover_m2())
    target_cover_m2 = Float64(_INIT_COVER_FRACTION) * Float64(area_m2)
...
Base.@ccallable function kf_run_reef(
    ...
    try
        ...
        ensemble_params = _build_ensemble_params(area_m2, n_members)
